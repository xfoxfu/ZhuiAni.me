// based on https://github.com/berhir/AspNetCore.SpaYarp/blob/master/src/AspNetCore.SpaYarp/SpaProxyMiddleware.cs
...
public class SpaProxyMiddleware
{
    ...
    {
        if (context.GetEndpoint()?.DisplayName?.EndsWith("/{**catch-all}") != true) { await _next(context); return; }
